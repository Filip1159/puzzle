...
frame.on("ready", function() {
    ...
    label.x = label.y = 20;
    label.on("click", function(){console.log("clicking");});
            ...
            for (let i = 0; i < horizontalPieces; i++) {
                ...
                s.addEventListener("pressup", function(e) {
                    ...
                    if (xx < puzzleX+gap / 2 && xx > puzzleX-gap / 2 && yy < puzzleX+gap / 2 && yy > puzzleY-gap / 2) {
                        ...
                        {
                            var pane = new zim.Pane({width:600,label:"VERY NICE!", height:250, modal:false, displayClose:false});

                            var confirm = new zim.Button(120, 50, "ON", "green").center(pane).mov(0,70);
                ...
                var hint = new zim.Shape();//s.clone(true);
                ...
                hint.addTo(con,0);
                console.log("animate")
                s.animate({obj:{x:zim.rand(-offsetX,frame.width-offsetX-pieceWidth),y:zim.rand(-offsetY,frame.height-offsetY-pieceHeight)}});
            }
        }
        con.addTo(stage)
        stage.update()
